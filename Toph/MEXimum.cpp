...
PII query(int node, int b, int e, int i, int j)
{
    ...
    PII p = query(left,b,mid,i,j);
    PII q = query(right,mid+1,e,i,j);
...
void dfs(int u, int p)
{
    ...
    if(curpos<=100000)
    {
        update(1,1,N,curpos,1);
        ans=max(ans,tree[1].second-1);
    }
    for(int v:adj[u])
    {
        if(v!=p)
    ...
    if(curpos<=100000) update(1,1,N,curpos,0);
}
