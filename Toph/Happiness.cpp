...
int n, m, dp[101][100005], arr[101], brr[101];
int visited[101][100005], cnt;

int fun(int pos, int expense)
{
    if(pos > n)
    {
        if(expense <= m) return 0;
        else return -1e9;
    }
    if(visited[pos][expense] == cnt) return dp[pos][expense];
    int val = 0;
    if(expense+arr[pos] <= m) val = max(val, fun(pos+1,expense+arr[pos])+brr[pos] );
    val = max(val, fun(pos+1,expense) );
    return dp[pos][expense] = val;
}
