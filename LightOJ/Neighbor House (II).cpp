...
int fun(int pos, int nisi)
{
    ...
    if(dp[pos][nisi] != -1) return dp[pos][nisi];
    int val = 0;
    if(pos==1)
    {
        val = max(val, fun(pos+2,1)+arr[pos] );
        val = max(val, fun(pos+1,nisi));
    }
    else
    {
        val = max(val, fun(pos+2,nisi)+arr[pos] );
        val = max(val, fun(pos+1,nisi));
    }
    return dp[pos][nisi] = val;
}
int main()
{
