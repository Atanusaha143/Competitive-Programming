...
int get_num(char c) /// converting number
{
...
LL fun(int mask, int mod)
{
    ...
    {
        if(!checkBit(mask,i)) /// checking if ith digit is free to take in our permutaion
        {
            int curr_number = get_num(str[i]);
            val += fun( setBit(mask,i),((mod*base)+curr_number)%k ); /// marking ith digit as taken and calculated the mod in respective base
        }
    }
