...
void solve(int casenum)
{
    ...
    int n = str.size();
    DH.make_hash(str);
    LL fhashval = DH.range_dhash(0,n-1);
    LL bhashval = DH.reverse_dhash(0,n-1);
    if(fhashval == bhashval) cout<<str.size()<<endl;
    else
    {
        for(int i=0; i<n; i++)
        {
            fhashval = DH.range_dhash(i,n-1);
            bhashval = DH.reverse_dhash(i,n-1);
            if(fhashval == bhashval)
            {
                int ans = str.size() + i;
                cout<<ans<<endl;
                break;
