...
LL fun(int pos, int agerta, int tar_agerta)
{
    if(pos > n)
    {
        if(agerta != 2) return 1;
        return 0;
    }
    if(dp[pos][agerta][tar_agerta] != -1) return dp[pos][agerta][tar_agerta];
    LL val = 0;
    if(pos == 1)
    {
        val += fun(pos+1,1,1);
    ...
    else
    {
        if(agerta == 1)
        {
            val += fun(pos+1,3,agerta);
            val += fun(pos+1,2,agerta);
        }
        else if(agerta == 2)
        {
            if(tar_agerta == 1)
            ...
            else if(tar_agerta == 3)
            {
                val += fun(pos+1,1,agerta);
            }
        }
        else if(agerta == 3)
        {
    ...
    return dp[pos][agerta][tar_agerta] = val;
}
