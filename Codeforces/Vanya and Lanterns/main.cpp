...
int main()
{
    ...
    bool checkstart = binary_search(all(arr),0);
    bool checkend = binary_search(all(arr),l);
    ...
    else if(checkstart and !checkend)
    {
        D s = l - arr[sz(arr)-1];
        D half = D(maxi)/2;
        D ans = max(half,s);
    ...
    {
        D f = (arr[0]-0)/2;
        D s = (l - arr[sz(arr)-1])/2;
        D ans1 = max(f,s);
        D ans2 = D(maxi)/2;
        printf("%0.10f", max(ans1,ans2));
    }
    else
    {
        D f = arr[0]-0;
