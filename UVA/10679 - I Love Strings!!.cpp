...
Link : https://onlinejudge.org/external/106/10679.pdf
Sol : Store unique lengths appears in query strings. For each length x, generate every possible
substring of length's hash value. Then for each query value's hash, check if we have found that earlier.
