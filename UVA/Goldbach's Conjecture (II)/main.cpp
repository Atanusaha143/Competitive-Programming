...
int main()
{
    ...
    while(sfi(n) and n)
    {
        vector <int> currPrime;
        bool vis[1000006];
        mem(vis,true);
        for(int i=0; i<=n; i++)
        {
            if(vecPrime[i]>n) break;
            else currPrime.PB(vecPrime[i]);
        }

        int count = 0;
        for(int i=0; i<sz(currPrime); i++)
        {
            int diff = n - currPrime[i];
            if(vis[currPrime[i]] and vis[diff] and binary_search(all(currPrime), diff))
            {
                count++;
                vis[currPrime[i]] = false;
                vis[diff] = false;
            }
        }
