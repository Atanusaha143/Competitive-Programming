...
void solve(int casenum)
{
    ...
    // As we have all values from 1 to k
    for(int i=1; i<=k; i++)
    {
        for(int j=i; j<=k; j+=i)
        {
    ...
    for(int i=k; i>=1; i--)
    {
        g[i]=power_mod(f[i],n,MOD); // we can have k^n configuration if we want to place k numbers in n places.
    ...
    for(int i=1; i<=k; i++)
    {
        ans+=mod_mul(g[i],i,MOD); // g[i] = number of subsequecnes having gcd i.
        ans%=MOD;
    }
    cout<<ans;
}
