...
//    int t; sfi(t); for(int tt=1; tt<=t; tt++){}
    ...
    cin>>n>>m;
    vector <int> original_vec1, original_vec2;
    int n1 = n / 2;
    int n2 = n - n1;
    for(int i=0; i<n; i++)
    {
        int x;
        cin>>x;
        if(i < n1) original_vec1.PB(x);
        else original_vec2.PB(x);
    }

    int limit1 = (1 << original_vec1.size());
    int limit2 = (1 << original_vec2.size());
    vector <LL> v1, v2;
    for(int mask=0; mask<limit1; mask++)
    {
        LL sum = 0;
        for(int i=0; i<original_vec1.size(); i++)
        {
            if(checkBit(mask,i))
            {
                sum += original_vec1[i];
    ...
    {
        LL sum = 0;
        for(int i=0; i<original_vec2.size(); i++)
        {
            if(checkBit(mask,i))
            {
                sum += original_vec2[i];
