...
LL fun(LL pos, LL prev)
{
    ...
    if(dp[pos][prev] != -2e9) return dp[pos][prev];

    if(prev == 0)
    {
        LL val1 = fun(pos+1,0);
        LL val2 = fun(pos+1,1) + arr[pos];
        LL val3 = fun(n+1,1) + arr[pos];
        return dp[pos][prev] = max({val1,val2,val3});
    }
    else
    {
        LL val1 = fun(pos+1,1) + arr[pos];
        LL val2 = fun(n+1,1) + arr[pos];
