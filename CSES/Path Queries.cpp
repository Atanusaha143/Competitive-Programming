...
vector <LL> adj[N];
LL startTime[N], finishTime[N], flatTree[2*N], cost[N], currTime;
LL tree[8*N], treeCost[8*N];
bool visited[N];
...
//    int t; sfi(t); For(tt,1,t){}
      ...
      while(q--)
      {
          ...
          if(k == 1)
          {
             LL node, value;
             sfl2(node,value);
             update(1LL,1LL,2*n,startTime[node],value);
             update(1LL,1LL,2*n,finishTime[node],-value);
          }
          else
          {
            LL node;
            sfl(node);
            LL ans = query(1LL,1LL,2*n,startTime[1], startTime[node]);
            printf("%lld\n", ans);
          }
      }
...
//      cout<<NL;
//      for(int i=1; i<=2*n; i++) cout<<flatTree[i]<<" ";
